...
prices.forEach((elem) => {
  ...
  div.className =
    "flex items-center justify-between flex px-[70px] py-[22px]  h-full";
  projectEl.className = `w-[350px]`;
  projectButton.className =
    "btn  btn-shad text-center py-3 w-[250px] bg-[#00BB34] cursor-pointer inline-block text-white font-bold rounded-[10px]";
  ...
  projectPrice.innerHTML = `Ремонт от <span class='text-3xl text-[#FF5C00] font-bold'>${elem.price} руб</span>`;
...
lastDiv.innerHTML = `<div class="px-[70px] py-[22px] font-normal text-lg italic underline">Подробные цены на ремонт стиральных машин</div>`;
...
listInstruction.forEach((elem) => {
  ...
  parElem.className =
    "py-5 relative w-[602px] rounded-[27px] flex items-center justify-center mb-11 bg-line";
  title.className = "text-white font-bold";
  child.className =
    "bg-white rounded-[50%] h-[95px] w-[95px] border border-[#5F24A8] flex justify-center items-center absolute left-0";
  child.append(imgs);
